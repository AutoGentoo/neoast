...
void CodeGenImpl::init_cc()
{
    ...
    if (input->has_errors())
    {
        input->emit_message(nullptr, "Failed to initialize canonical collection");
        return;
    }
    ...
    if (input->has_errors())
    {
        input->emit_message(nullptr, "Failed to resolve canonical collection");
        return;
    }
    ...
    if (error)
    {
        input->emit_message(nullptr, "Failed to generate parsing table");
        return;
    }
}
