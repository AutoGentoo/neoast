...
const char* tok_names = "$ohdtslrumxei|;{FKHLGMTSMA";
...
static I32
ll_macro(const char* lex_text, CodegenUnion* lex_val)
{
...
static I32
ll_header(const char* lex_text, CodegenUnion* lex_val, U32 len)
{
...
static I32
ll_l_rule(const char* lex_text, CodegenUnion* lex_val, U32 len)
{
    // Bracket counters
...
static I32
ll_g_rule(const char* lex_text, CodegenUnion* lex_val, U32 len)
{
...
static I32
ll_g_function (const char* lex_text, CodegenUnion* lex_val, U32 len)
{
...
static I32
ll_g_tok (const char* lex_text, CodegenUnion* lex_val, U32 len)
{
    lex_val->token = token_build(strndup(lex_text, len));
    return TOK_G_TOK;
}

#define KEY_VAL_REGEX "[A-z][A-z|0-9]*=\"[A-z|0-9_\\-\\+]*\""

static LexerRule ll_rules_s0[] = {
        {.regex_raw = "^[\n ]+"}, // skip
        {.regex_raw = "^%%", .expr = (lexer_expr) ll_delim},
        {.regex_raw = "^==", .expr = (lexer_expr) ll_delim},
        {.regex_raw = "^%option " KEY_VAL_REGEX, .expr = (lexer_expr) ll_option},
...
const U32 grammars[][7] = {
        ...
        {TOK_LEFT},
        {TOK_RIGHT},
        {TOK_GG_KEY_VALS, TOK_GG_KEY_VALS},
...
static const GrammarRule gg_rules[] = {
        ...
        {.token=TOK_GG_KEY_VALS, .tok_n=1, .grammar=grammars[2]},
        {.token=TOK_GG_KEY_VALS, .tok_n=1, .grammar=grammars[3]},
        {.token=TOK_GG_KEY_VALS, .tok_n=1, .grammar=grammars[4]},
        {.token=TOK_GG_KEY_VALS, .tok_n=1, .grammar=grammars[5]},
        {.token=TOK_GG_KEY_VALS, .tok_n=1, .grammar=grammars[6]},
        {.token=TOK_GG_KEY_VALS, .tok_n=1, .grammar=grammars[7]},
        {.token=TOK_GG_KEY_VALS, .tok_n=2, .grammar=grammars[8], (parser_expr) gg_key_val_add_next},
        {.token=TOK_GG_HEADER, .tok_n=2, .grammar=grammars[9], (parser_expr) gg_build_header_1},
        {.token=TOK_GG_HEADER, .tok_n=1, .grammar=grammars[10]},
        {.token=TOK_GG_LEX_RULES, .tok_n=1, .grammar=grammars[11]},
        {.token=TOK_GG_LEX_RULES, .tok_n=2, .grammar=grammars[12], (parser_expr) gg_build_l_rule_1},
        {.token=TOK_GG_TOKENS, .tok_n=0, .grammar=grammars[13], (parser_expr) gg_build_tok_empty},
        {.token=TOK_GG_TOKENS, .tok_n=1, .grammar=grammars[14]},
        {.token=TOK_GG_TOKENS, .tok_n=2, .grammar=grammars[15], (parser_expr) gg_build_tok_1},
        {.token=TOK_GG_SINGLE_GRAMMAR, .tok_n=2, .grammar=grammars[16], (parser_expr) gg_build_single},
        {.token=TOK_GG_MULTI_GRAMMAR, .tok_n=1, .grammar=grammars[17]},
        {.token=TOK_GG_MULTI_GRAMMAR, .tok_n=3, .grammar=grammars[18], (parser_expr) gg_build_multi},
        {.token=TOK_GG_GRAMMAR, .tok_n=3, .grammar=grammars[19], (parser_expr) gg_build_grammar},
        {.token=TOK_GG_GRAMMARS, .tok_n=1, .grammar=grammars[20]},
        {.token=TOK_GG_GRAMMARS, .tok_n=2, .grammar=grammars[21], (parser_expr) gg_build_grammars},
        {.token=TOK_GG_FILE, .tok_n=7, .grammar=grammars[22], (parser_expr) gg_build_file},
};
...
int gen_parser_init(GrammarParser* self)
{
    ...
    self->grammar_n = ARR_LEN(grammars);
